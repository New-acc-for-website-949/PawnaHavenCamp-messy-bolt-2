  ...
  return (
    <div className="space-y-6 max-w-full sm:max-w-2xl mx-auto px-0 sm:px-4 pb-10">
      <div className="flex items-center justify-between px-4 sm:px-0">
        <h1 className="text-xl sm:text-2xl font-bold text-[#D4AF37] font-display">Manage Availability & Prices</h1>
      </div>
        ...
        <div className="bg-black/40 border border-[#D4AF37]/30 rounded-2xl p-6 sm:p-8 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label className="text-xs text-gray-400 uppercase tracking-widest">Weekday Price</Label>
            ...
            <div className="space-y-2">
              <Label className="text-xs text-gray-400 uppercase tracking-widest">Weekend Price</Label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#D4AF37]">₹</span>
                <Input 
                  type="number" 
          ...
          <div className="space-y-4 pt-4 border-t border-white/5">
            <div className="flex items-center justify-between">
              <Label className="text-xs text-gray-400 uppercase tracking-widest">Special Day Prices</Label>
              <Button 
                variant="outline" 
                size="sm" 
                onClick={() => setPrices({...prices, special: [...prices.special, {date: '', price: ''}]})}
              ...
              {prices.special.map((special, idx) => (
                <div key={idx} className="flex gap-2 items-center bg-white/5 p-3 rounded-xl border border-white/10">
                  ...
                  <div className="relative flex-1">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#D4AF37] text-xs">₹</span>
                    <Input 
                      type="number" 
                      value={special.price}
                      onChange={e => {
                  ...
                  <Button 
                    variant="ghost" 
                    size="sm" 
                    onClick={() => setPrices({...prices, special: prices.special.filter((_, i) => i !== idx)})}
